...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_not_end) { described_class.new(7, '8') }
      # remove the 'x' before running this test
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        wrong_number = '666'
        valid_input = '8'
        allow(game_loop).to receive(:player_input).and_return(letter, wrong_number, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game) { NumberGame.new(6, '6', 3) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
    ...
    context 'when count is 4 and over' do
      subject(:game2) { NumberGame.new(8, '8', 4) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        winner_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game2.final_message }.to output(winner_phrase).to_stdout
      end
    end
  end
end
