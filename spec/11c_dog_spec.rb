...
describe Dog do
  ...
  subject(:doggy) { Dog.new('Marshall', 'Dalmatian', 'White with black spots') }

  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when method name is the same in sibling classes' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(doggy.barking?).to be(false)
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(doggy.sleeping?).to be(true)
  end
end
