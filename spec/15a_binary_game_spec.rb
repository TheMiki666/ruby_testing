...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      subject(:game_input2) { described_class.new(1, 10) }
      before do
        wrong_input = '666'
        valid_input = '3'
        allow(game_input2).to receive(:gets).and_return(wrong_input, valid_input)
      end

      it 'completes loop and displays error message once' do
        min = game_input2.instance_variable_get(:@minimum)
        max = game_input2.instance_variable_get(:@maximum)
        error_message = "Input error! Please enter a number between #{min} or #{max}."
        expect(game_input2).to receive(:puts).with(error_message).once
        game_input2.player_input(min, max)
      end
    end

    context 'when user inputs two incorrect values, then a valid input' do
      subject(:game_input3) { described_class.new(1, 10) }
      before do
        wrong_input1 = '666'
        wrong_input2 = '-18'
        valid_input = '3'
        allow(game_input3).to receive(:gets).and_return(wrong_input1, wrong_input2, valid_input)
      end

      it 'completes loop and displays error message twice' do
        min = game_input3.instance_variable_get(:@minimum)
        max = game_input3.instance_variable_get(:@maximum)
        error_message = "Input error! Please enter a number between #{min} or #{max}."
        expect(game_input3).to receive(:puts).with(error_message).twice
        game_input3.player_input(min, max)
      end
    end
  end
  ...
  subject(:verify_my_input) { described_class.new(1, 10) }
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        expect(verify_my_input.verify_input(2,8,5)).to eql(5)
      end
    end

    context 'when given invalid input as argument' do
      it 'returns nil' do
        expect(verify_my_input.verify_input(2,8,9)).to be_nil
      end
    end
  end
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      subject(:game_half_thousand) {described_class.new(100,600)}
      it 'returns 9' do
        max = game_half_thousand.maximum_guesses
        expect(max).to eq(9)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    let(:searcher) { instance_double(BinarySearch) }
    before do
      # You'll need to create a few method stubs.
      allow(searcher).to receive(:make_guess)
      allow(searcher).to receive(:update_range)
    ...
    it 'increases guess_count by one' do
      expect { turn_order.display_turn_order(searcher) }.to change { turn_order.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(searcher).to receive(:make_guess).once
      turn_order.display_turn_order(searcher)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(searcher).to receive(:update_range).once
      turn_order.display_turn_order(searcher)
    end
