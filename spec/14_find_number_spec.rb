...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        expect(game_guessing.make_guess(0, 9)).to eql(4)
      end
    end
    ...
    context 'when min is 5 and max is 9' do
      subject(:game1) { described_class.new(0, 9, number_guessing) }
      it 'returns 7' do
        expect(game1.make_guess(5, 9)).to eql(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game2) { described_class.new(0, 9, number_guessing) }
      it 'returns 8' do
        expect(game2.make_guess(8, 9)).to eql(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game3) { described_class.new(0, 9, number_guessing) }
      it 'returns 1' do
        expect(game3.make_guess(0, 3)).to eql(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game4) { described_class.new(0, 9, number_guessing) }
      it 'returns 3' do
        expect(game4.make_guess(3, 3)).to eql(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    let(:dice_toss) { double('random_number', value: 5) }
    subject(:my_game1) { described_class.new(0, 9, dice_toss) }
    context 'when guess and random_number are equal' do
      ...
      it 'is game over' do
        expect(my_game1.game_over?(5)).to be(true)
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      it 'is not game over' do
        expect(my_game1.game_over?(6)).to be(false)
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        low_guess_game.update_range(4)
        expect(low_guess_game.min).to eql(5)
      end

      it 'does not update max' do
        low_guess_game.update_range(4)
        expect(low_guess_game.max).to eql(9)
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        high_guess_game.update_range(9)
        expect(high_guess_game.min).to eql(0)
      end

      it 'updates max to 8' do
        high_guess_game.update_range(9)
        expect(high_guess_game.max).to eql(8)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      let(:secret_number) { double('random_number', value: 8) }
      subject(:low_and_high_guess_game) { described_class.new(5, 8, secret_number) }
      it 'updates min to the same value as max' do
        low_and_high_guess_game.update_range(7)
        expect(low_and_high_guess_game.min).to eql(low_and_high_guess_game.max)
      end

      it 'does not update max' do
        low_and_high_guess_game.update_range(7)
        expect(low_and_high_guess_game.max).to eql(8)
      end
    end
  end
end
